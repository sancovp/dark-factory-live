---
name: 0.8.4-understand-happens_before
description: "[0.8.4] transitive closure of program_order and synchronizes_with relations; the ordering guarantee that prevents data"
---

# understand-happens_before

**CALL NUMBER:** `deep_synchronizes_with.happens_before : deep_happens_before_relat(7), concurrency_synchronization_primitives_and_memor(2), deep_c11_memory_model(1)`
**DEFINITION:** transitive closure of program_order and synchronizes_with relations; the ordering guarantee that prevents data races

Invoke this skill to understand `happens_before` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.
...
- **program_order** (d1): The sequential ordering of operations within a single thread as written in the source code, before any concurrent interleaving is considered.
- **inter_thread_happens_before** (d2): The subset of happens_before relations that cross thread boundaries, established by synchronizes_with connections between release and acquire operations.
- **happens_before_order** (d3): A transitive, irreflexive partial order over operations in a memory model that defines which operations must appear to precede others from any thread's perspective.
...
`c11_atomic`, `dependency_chain`, `reads_from`
