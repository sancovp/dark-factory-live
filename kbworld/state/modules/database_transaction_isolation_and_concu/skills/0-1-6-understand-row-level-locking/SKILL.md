---
name: 0.1.6-understand-row_level_locking
description: "[0.1.6] Lock management at the individual row granularity via row_lock entries in lock_manager's lock table; enables h"
---

# understand-row_level_locking

**CALL NUMBER:** `database_transaction_isolation_and_concurrency_c.row_level_locking : deep_database_transaction(6), deep_isolation_level(1)`
**DEFINITION:** Lock management at the individual row granularity via row_lock entries in lock_manager's lock table; enables high concurrency but increases lock_manager overhead.

Invoke this skill to understand `row_level_locking` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.
...
- **committed_data** (d5): database state resulting from successfully completed transactions; constitutes the visible universe for read_committed queries

## CONSUMERS (what needs this)
`lock_timeout_handling`, `non_repeatable_read_possible`, `phantom_read_possible`, `read_lock_release`, `write_lock_held_until_commit`
