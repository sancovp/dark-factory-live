---
name: 0.4.6-understand-lock_timeout_handling
description: [0.4.6] timeout mechanism in read_committed for transactions blocked waiting to acquire conflicting locks on rows
---

# understand-lock_timeout_handling

**CALL NUMBER:** `deep_isolation_level.lock_timeout_handling : deep_database_transaction(12), database_transaction_isolation_and_concurrency_c(11)`
**DEFINITION:** timeout mechanism in read_committed for transactions blocked waiting to acquire conflicting locks on rows

Invoke this skill to understand `lock_timeout_handling` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.
...
- **write_lock_held_until_commit** (d1): exclusive lock discipline in read_committed requiring a transaction to hold write locks on modified rows until transaction commit or rollback
