---
name: 0.3.1-understand-a005
description: "[0.3.1] Uniform agreement: all processes including those that later fail must decide on the same value, stricter than "
---

# understand-a005

**CALL NUMBER:** `deep_raft.a005 : distributed_consensus_and_replication_protocols(3)`
**DEFINITION:** Uniform agreement: all processes including those that later fail must decide on the same value, stricter than ordinary agreement which only binds correct processes.

Invoke this skill to understand `a005` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.
...
`a004`, `a006`, `ft011`, `replicated_log`
