---
name: 0.4.1-understand-command_message
description: "[0.4.1] A message that encodes an invocation of an operation on the receiver, carrying method name and arguments."
---

# understand-command_message

**CALL NUMBER:** `enterprise_integration_patterns.command_message : deep_message(9)`
**DEFINITION:** A message that encodes an invocation of an operation on the receiver, carrying method name and arguments.

Invoke this skill to understand `command_message` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.

## THE RELATIVE ROOT (the import cone, by lib)

### from `?`
- **command_priority** (d1): A metadata field indicating the urgency or precedence level of a command message, used by schedulers and routers to order or filter command processing.
- **command_target** (d1): The logical or physical destination endpoint to which a command message is addressed, specifying which receiver should process the invocation.
- **command_timeout** (d1): The maximum duration a sender will wait for a command to complete before considering it failed; exceeding this triggers timeout handling or retry.
- **execution_context** (d1): A structured record of runtime state passed alongside a command, providing the environment information needed to correctly process the operation invocation.
...
- **idempotency_key** (d1): A unique identifier assigned to a command that allows receivers to detect and discard duplicate submissions, ensuring safe retries without double execution.
- **operation_arguments** (d1): The parameter values supplied to the operation being invoked by a command message, structured according to the target interface's argument schema.
- **operation_identifier** (d1): A named or coded reference that uniquely identifies which operation a command message is requesting, mapping to a procedure on the target system.
...
- **target_interface_version** (d1): A version descriptor indicating which contract version of the target interface a command message adheres to, enabling version-aware routing and validation.
...
- **dm_007** (d3): document_origin: Metadata identifying the originating system, agent, or process that produced the document_message.
...
- **idempotent_consumer** (d1): A consumer designed to safely process the same message more than once without side effects, essential for guaranteed delivery.
...
- **message_store** (d1): A durable repository of all messages processed by an integration component, enabling replay and audit trails.
...
- **message_historian** (d2): A component that archives all messages passing through a channel for compliance, auditing, or forensic replay.
...
- **aggregator** (d3): A router that collects related messages over time and combines them into a single coherent output message.
- **bulk_message_consumer** (d3): A consumer that collects multiple messages over a batching window and processes them together for throughput optimization.
- **resequencer** (d4): A router that reorders a stream of messages into the correct sequence before passing them downstream.

## CONSUMERS (what needs this)
`cmd_001`, `cmd_002`, `cmd_003`, `cmd_004`, `cmd_005`, `cmd_006`, `cmd_010`, `message`
