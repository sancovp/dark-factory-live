---
name: 0.7.1-understand-c11_atomic
description: [0.7.1] The C11/C++11 memory model defined by the ISO standard; defines six memory orders (relaxed, consume, acquire, 
---

# understand-c11_atomic
...
**DEFINITION:** The C11/C++11 memory model defined by the ISO standard; defines six memory orders (relaxed, consume, acquire, release, acq_rel, seq_cst) and formalizes synchronizes-with and happens-before.

Invoke this skill to understand `c11_atomic` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.
...
- **acquire_semantics** (d4): Memory ordering semantics ensuring all subsequent loads/stores cannot be reordered before the acquire operation; used for lock acquisition, reading a flag after volatile write.
...
- **intermediate_state_invisibility** (d3): The guarantee that no thread can observe a partially completed relaxed atomic operation; all threads observe either the state before or after the operation, never during its execution.
...
- **no_partial_observation** (d3): The constraint that observers of a relaxed atomic operation cannot see byte-level or word-level fragments of the operation in flight; only complete before or after states are visible.
- **read_modify_write_indivisibility** (d3): The indivisibility of compound read-modify-write operations under relaxed ordering; fetch_add fetch_sub and similar operations complete atomically without observable intermediate states.
...
- **atomic_variable_scope** (d3): The scope boundary of atomicity guarantees limited to the specific atomic variable being read or written; other variables in the program are unaffected by this atomicity property.
- **memory_order_relaxed_vs_acquire** (d3): The ordering contrast where acquire_semantics prevents subsequent operations from being reordered before the acquire, while memory_order_relaxed permits all such reorderings with no barrier effect.
- **memory_order_relaxed_vs_release** (d3): The ordering contrast where release_semantics prevents prior operations from being reordered after the release, while memory_order_relaxed permits all such reorderings with no barrier effect.
- **memory_order_relaxed_vs_seq_cst** (d3): The ordering contrast where sequentially_consistent imposes a single global total order visible to all threads, while memory_order_relaxed has no global ordering requirement across variables.
- **per_variable_modification_order_integrity** (d3): The per-variable constraint that each atomic variable maintains a single agreed modification order even under relaxed operations; atomicity preserves this per-variable coherence order.
...
- **happens_before** (d1): transitive closure of program_order and synchronizes_with relations; the ordering guarantee that prevents data races
- **memory_order_acq_rel** (d1): A memory ordering that is simultaneously acquire and release; applies to read-modify-write atomics and makes the update visible atomically with surrounding writes.
...
- **memory_order_seq_cst** (d1): The strongest memory ordering; both a global total order of all seq_cst operations exists and the program order and synchronization order coincide, making reasoning straightforward but costly.
