...
1. **dependency_lens** — Maps what the guard under construction requires to exist in loadout.
2. **convergence_lens** — Verifies the guard doesn't duplicate existing guards (monoculture risk).
...
This recipe itself requires dependency_lens and convergence_lens. Both exist in the crafted folder. Their dependencies are self-contained (they don't import other skills). This recipe has proven its own composition.
