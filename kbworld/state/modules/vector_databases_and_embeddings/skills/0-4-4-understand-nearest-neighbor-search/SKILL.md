---
name: 0.4.4-understand-nearest_neighbor_search
description: "[0.4.4] Query operation returning the k closest vectors by a distance or similarity metric in embedding space."
---

# understand-nearest_neighbor_search

**CALL NUMBER:** `vector_databases_and_embeddings.nearest_neighbor_search`
**DEFINITION:** Query operation returning the k closest vectors by a distance or similarity metric in embedding space.

Invoke this skill to understand `nearest_neighbor_search` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.
...
`maximum_marginal_relevance`, `recommendation_system`, `reranking`, `semantic_search`, `vector_embedding`
