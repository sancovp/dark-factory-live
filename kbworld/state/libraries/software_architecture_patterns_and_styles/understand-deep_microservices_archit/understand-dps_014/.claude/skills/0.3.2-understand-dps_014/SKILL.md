---
name: 0.3.2-understand-dps_014
description: [0.3.2] eventual_consistency_model is the consistency guarantee that results from database_per_service: writes are imm
---

# understand-dps_014

**CALL NUMBER:** `deep_microservices_archit.dps_014 : deep_event_driven_archite(9), software_architecture_patterns_and_styles(8)`
**DEFINITION:** eventual_consistency_model is the consistency guarantee that results from database_per_service: writes are immediately durable in one service's database, and other services receive updates asynchronously via event_streaming or message_queue.

Invoke this skill to understand `dps_014` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.
