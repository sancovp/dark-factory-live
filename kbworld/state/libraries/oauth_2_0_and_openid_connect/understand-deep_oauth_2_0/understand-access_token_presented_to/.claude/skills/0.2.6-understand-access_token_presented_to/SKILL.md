---
name: 0.2.6-understand-access_token_presented_to
description: [0.2.6] The resource_server that receives and validates the access_token on each protected API call; the token is forw
---

# understand-access_token_presented_to

**CALL NUMBER:** `deep_oauth_2_0.access_token_presented_to : oauth_2_0_and_openid_connect(10)`
**DEFINITION:** The resource_server that receives and validates the access_token on each protected API call; the token is forwarded via HTTP Authorization header using the bearer scheme.

Invoke this skill to understand `access_token_presented_to` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.
