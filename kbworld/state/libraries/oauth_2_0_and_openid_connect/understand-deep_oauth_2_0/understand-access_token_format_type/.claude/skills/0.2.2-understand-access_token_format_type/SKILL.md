---
name: 0.2.2-understand-access_token_format_type
description: [0.2.2] Classification of the access_token internal structure: opaque (random string referencing server-side state) or
---

# understand-access_token_format_type

**CALL NUMBER:** `deep_oauth_2_0.access_token_format_type : oauth_2_0_and_openid_connect(10)`
**DEFINITION:** Classification of the access_token internal structure: opaque (random string referencing server-side state) or structured (JWT containing claims about scope, audience, expiration embedded directly).

Invoke this skill to understand `access_token_format_type` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.
