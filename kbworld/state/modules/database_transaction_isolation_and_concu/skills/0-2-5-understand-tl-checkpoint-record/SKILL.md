---
name: 0.2.5-understand-tl_checkpoint_record
description: "[0.2.5] A special log_record written periodically by a checkpoint_writer; records the volatile database state at a con"
---

# understand-tl_checkpoint_record

**CALL NUMBER:** `deep_database_transaction.tl_checkpoint_record`
**DEFINITION:** A special log_record written periodically by a checkpoint_writer; records the volatile database state at a consistent point, bounding the recovery_manager's replay horizon.

Invoke this skill to understand `tl_checkpoint_record` down to its primitives. The RELATIVE ROOT below is the least-fixed-point closure of everything it bundles from — the full import cone, grouped by the lib each prim comes from. Projected from a prover-typed KB (MAP/SWI-Prolog consistency gate): every reference below resolves.
...
- **tl_redo_log** (d3): A log_record or series of tl_update_record entries capturing the after_image of modifications; replayed by recovery_manager to idempotently reconstruct committed changes after a crash.
- **tl_undo_log** (d3): A structured collection of tl_update_record entries chained by a transaction; traversed in reverse order during rollback to restore the before_image of each modified entity.
...
`tl_checkpoint_writer`, `tl_recovery_manager`
